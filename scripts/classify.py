...
def classify_players(df, role_config, hybrid_threshold=5.0):
    ...
    for broad_pos, config in role_config.items():
        ...
        # ── Step 3: Assign best role — hybrid baked into the label itself
        def assign_best_role(row):
            ...
            if len(sorted_roles) >= 2:
                gap = scores[sorted_roles[0]] - scores[sorted_roles[1]]
                if gap < hybrid_threshold:
                    second_role = sorted_roles[1]
                    return f"{best_role} / {second_role}"  # e.g. "Poacher / Target Man"

            return best_role  # e.g. "Poacher"
    ...
    # ── Reorder columns cleanly — no Role_Clarity needed
