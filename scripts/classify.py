import pandas as pd
from scripts.config import gk_config, outfield_config
...
def classify_players(df, role_config, hybrid_threshold=5.0):
    ...
    for broad_pos, config in role_config.items():
        ...
        for feature in all_features:
            if feature in pos_df.columns:
                ...
                pos_df[f"{feature}_Pct"] = (
                    pos_df[feature].fillna(median_val).rank(pct=True).clip(0, 1)
                )
            else:
        ...
        for role, features in role_map.items():
            ...
            pos_df[f"{role}_Score"] = (
                pos_df[pct_cols].mul(norm_weights, axis=1).sum(axis=1) * 100
            )
        ...
        def assign_best_role(row):
            ...
            return pd.Series(
                [best_role, clarity], index=["Tactical_Role", "Role_Clarity"]
            )

        pos_df[["Tactical_Role", "Role_Clarity"]] = pos_df.apply(
            assign_best_role, axis=1
        )
    ...
    meta_cols = ["Broad_Position", "Position", "Tactical_Role", "Role_Clarity"]
    other_cols = [
        c for c in result.columns if c not in score_cols and c not in meta_cols
    ]
