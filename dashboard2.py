...
.feature-card{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:2rem;position:relative;overflow:hidden;transition:border-color 0.3s;}
...
if page == "🏠  Overview":
    ...
    features = [
        {"accent":"#00e676","top":"2px solid #00e676","emoji":"🔍","title":"Player Explorer",
         "desc":"Search and filter all 735 players by position, tactical role, age, and league. Includes a full goalkeeper mode with GK-specific filters and sorting.",
         "pills":["735 Players","7 Positions","46 Roles","GK Mode"]},
        {"accent":"#00b0ff","top":"2px solid #00b0ff","emoji":"👤","title":"Player Profile",
         "desc":"Deep dive into any player — radar chart vs position peers, role fit scores, a full stat breakdown across 5 categories, and market value vs ML prediction.",
         "pills":["Radar Chart","Role Scores","5 Stat Tabs","Value Gap"]},
        {"accent":"#ffd740","top":"2px solid #ffd740","emoji":"🕵️","title":"Scouting Report",
         "desc":"Six curated watchlists: Top Performers, On Fire, Rising Stars (U21), Undervalued Gems, Defensive Walls, and Best Value for Money. Data-driven, not opinion-driven.",
        ...
        {"accent":"#ff5252","top":"2px solid #ff5252","emoji":"💰","title":"Market Value Intelligence",
         "desc":"ML predictions for every player's true market value. Discover who the market is sleeping on, who is overpriced, and where the smartest signings are hiding.",
    ...
    for i, f in enumerate(features):
        ...
        ac  = f["accent"]
        pills = "".join([f"<span style='display:inline-block;border:1px solid {ac}55;color:{ac};background:{ac}11;border-radius:100px;padding:2px 10px;font-size:0.68rem;font-weight:600;letter-spacing:0.08em;margin:2px'>{p}</span>" for p in f["pills"]])
        col.markdown(f"<div style='background:#0f1419;border:1px solid #1e2a38;border-top:{f['top']};border-radius:12px;padding:1.4rem 1.6rem;margin-bottom:1rem'><div style='font-size:1.5rem;margin-bottom:0.5rem'>{f['emoji']}</div><div style='font-family:Bebas Neue,sans-serif;font-size:1.3rem;letter-spacing:0.06em;color:{ac};margin-bottom:0.5rem'>{f['title']}</div><div style='font-size:0.85rem;color:#7a8fa0;line-height:1.6;margin-bottom:1rem'>{f['desc']}</div><div>{pills}</div></div>", unsafe_allow_html=True)
    ...
    audiences = [
        ...
        ("📰","Journalists & Writers","Reporters who want facts, rankings, and market data to back their stories."),
    ...
    for i, (emoji, title, desc) in enumerate(audiences):
        col = [a1, a2, a3][i % 3]
        col.markdown(f"<div style='background:#0f1419;border:1px solid #1e2a38;border-radius:10px;padding:1.1rem 1.2rem;margin-bottom:0.8rem'><div style='font-size:1.2rem;margin-bottom:5px'>{emoji}</div><div style='font-family:Bebas Neue,sans-serif;font-size:1rem;letter-spacing:0.05em;color:#e8edf2;margin-bottom:4px'>{title}</div><div style='font-size:0.78rem;color:#5a6a7a;line-height:1.5'>{desc}</div></div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    <div style='background:#0f1419;border:1px solid #1e2a38;border-radius:12px;padding:1.8rem 2rem;margin-bottom:1.5rem;display:flex;align-items:center;gap:2rem'>
        <div style='width:72px;height:72px;background:#161d26;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2rem;flex-shrink:0'>{"🥅" if is_gk else "⚽"}</div>
        <div style='flex:1'>
        ...
        <div style='text-align:right'>
            <div style='font-family:Bebas Neue,sans-serif;font-size:1.1rem;color:#00e676;letter-spacing:0.08em'>{position_display}</div>
            <div style='font-size:0.85rem;color:#5a6a7a;margin-top:2px'>{role_display}</div>
    ...
    with col_l:
        if not is_gk:
            section_header("Performance Radar", "Normalised per-90 stats vs players in the same position (95th percentile = 1.0)")
            stat_cols_map = {
            ...
            section_header("Goalkeeper Radar", "Normalised stats vs all goalkeepers in the dataset (95th percentile = 1.0)")
    ...
    with col_r:
        if not is_gk:
            ...
            if gk_scores:
                ...
                role_desc = {
                    "Shot Stopper":   "Excels at shot-stopping, reflexes, and commanding the penalty area. Strong between the posts.",
                    "Sweeper Keeper": "Comfortable with the ball at feet, sweeps behind the defensive line, and acts as an extra outfield player.",
                }
                st.markdown(f"""
                <div style='background:#0f1419;border:1px solid #1e2a38;border-radius:10px;padding:1rem 1.2rem;margin-top:0.5rem'>
                    <div style='font-size:0.65rem;color:#5a6a7a;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:4px'>Primary Role</div>
                    <div style='font-family:Bebas Neue,sans-serif;font-size:1.2rem;color:#00b0ff'>{primary_role} <span style='font-size:0.9rem;color:#5a6a7a'>· {primary_score:.0f}/100</span></div>
                    <div style='font-size:0.78rem;color:#7a8fa0;margin-top:6px;line-height:1.5'>{role_desc.get(primary_role,'')}</div>
                </div>""", unsafe_allow_html=True)
            else:
    ...
    with tab_att:
        h = tbl_h+stat_row("Goals Scored",row.get("Goals Scored"),row.get("Goals Scored (Per90)"))+stat_row("Shots Taken",row.get("Shots Taken"),row.get("Shots Taken (Per90)"))+stat_row("Shots On Target",row.get("Shots On Target"),row.get("Shots On Target (Per90)"))+stat_row("Shot Accuracy",row.get("Shot Accuracy"))+stat_row("Goal Involvement",row.get("Goal Involvement"),row.get("Goal Involvement (Per90)"))+"</table>"
        st.markdown(h, unsafe_allow_html=True)
    with tab_def:
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    <div style='display:flex;align-items:baseline;gap:0.8rem;margin:2.2rem 0 0.3rem 0'>
        ...
        <div style='font-size:0.78rem;color:#5a6a7a'>Top shot stoppers and sweeper keepers across both leagues</div>
    </div>
    ...
    with gk_col1:
        ...
        if not best_stoppers.empty:
            for _, gk_row in best_stoppers.iterrows():
                sp = gk_row.get("Save Percentage", 0)
                sp_str = f"{sp*100:.1f}%" if pd.notna(sp) and sp <= 1 else (f"{sp:.1f}%" if pd.notna(sp) else "—")
            ...
            st.markdown("<div style='color:#5a6a7a;font-size:0.85rem'>No data available for current filter.</div>", unsafe_allow_html=True)
